...
#ifdef GNUTLS_VER_2_X
		const char protocol_priority[] = { GNUTLS_TLS1, GNUTLS_SSL3, 0 };
		const char cipher_priority[] = { GNUTLS_CIPHER_3DES_CBC, GNUTLS_CIPHER_ARCFOUR, 0};
		const char comp_priority[] = { GNUTLS_COMP_ZLIB, GNUTLS_COMP_NULL, 0 };
		const char kx_priority[] = { GNUTLS_KX_RSA, 0 };
		const char mac_priority[] = { GNUTLS_MAC_SHA, GNUTLS_MAC_MD5, 0 };
		const char *err_pos;
		gnutls_priority_set_direct(session->tls_session, protocol_priority, &err_pos);
		gnutls_priority_set_direct(session->tls_session, cipher_priority, &err_pos);
		gnutls_priority_set_direct(session->tls_session, comp_priority, &err_pos);
		gnutls_priority_set_direct(session->tls_session, kx_priority, &err_pos);
		gnutls_priority_set_direct(session->tls_session, mac_priority, &err_pos);
#else
